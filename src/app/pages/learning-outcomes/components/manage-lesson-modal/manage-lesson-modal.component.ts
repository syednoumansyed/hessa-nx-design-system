...
@Component({
  selector: 'app-manage-lesson-modal',
  templateUrl: './manage-lesson-modal.component.html',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, DsFormRendererComponent],
})
export class ManageLessonModalComponent
  implements OnInit, DsModalContentComponent
{
  ...
  @Input() unitId!: number;
  @Input() unitName = '';
  @Input() lesson?: LearningOutcomeLesson;
    ...
    return [
      {
        ...
        label: this.hesTranslateService.t(
          'learning_outcome.lesson_name_en.label',
        ),
        placeholder: this.hesTranslateService.t(
          'learning_outcome.lesson_name_en.placeholder',
        ),
        required: true,
      },
      {
        ...
        label: this.hesTranslateService.t(
          'learning_outcome.lesson_name_ar.label',
        ),
        placeholder: this.hesTranslateService.t(
          'learning_outcome.lesson_name_en.placeholder',
        ),
  ...
  ngOnInit() {
    ...
    if (this.isEditMode && this.lesson) {
      this.form.patchValue({
        enName: this.lesson.enName,
        arName: this.lesson.arName,
      });
    }
  ...
  private createLesson() {
    ...
    this.learningOutcomesService.createLesson(payload).subscribe({
      next: (lesson) => {
        this.hesToaster.success(
          this.hesTranslateService.t('learning_outcome.lesson_added.msg'),
        );
        this.closeModal(
          { lesson, unitId: this.unitId, action: 'create' },
          'confirm',
        );
      },
      error: (err) => {
  ...
  private updateLesson() {
    ...
    this.learningOutcomesService
      .updateLesson(this.lesson!.id, payload)
      .subscribe({
        next: (lesson) => {
          this.hesToaster.success(
            this.hesTranslateService.t('learning_outcome.lesson_updated.msg'),
          );
          this.closeModal(
            { lesson, unitId: this.unitId, action: 'update' },
            'confirm',
          );
        },
        error: (err) => {
