...
const ALIAS_PREFIX = '@';
const isNonEmpty = (obj: any): boolean => {
...
export class BreadcrumbService {
  ...
  private detectRouteChanges() {
    ...
    this.router.events
      .pipe(
        filter(
          (event): event is GuardsCheckEnd | ResolveEnd =>
            event instanceof GuardsCheckEnd || event instanceof ResolveStart,
        ),
      )
      .subscribe((event) => {
  ...
  private prepareBreadcrumbItems(
    activatedRouteSnapshot: ActivatedRouteSnapshot,
    routeLinkPrefix: string,
  ): BreadcrumbDefinition[] {
    return (
      activatedRouteSnapshot.routeConfig?.data?.['breadcrumbs']?.map(
        (breadcrumb: any) => {
          const resolvedSegment = this.resolvePathSegment(
            breadcrumb.path,
            activatedRouteSnapshot,
          );
          const routeLink = `${breadcrumb.routeLink}/`;
          const storeItem = this.getFromStore(
            breadcrumb.alias ?? '',
            routeLink,
          );

          const label = this.extractLabel(
            storeItem || breadcrumb,
            resolvedSegment,
          );
          let isAutoGeneratedLabel = false;
          let autoGeneratedLabel = '';
  ...
  private prepareBreadcrumbList(
    ...
    if (activatedRouteSnapshot.routeConfig?.path) {
      let breadcrumbItems: BreadcrumbDefinition[] = [];

      if (activatedRouteSnapshot?.routeConfig?.data?.['breadcrumbs']) {
        breadcrumbItems = this.prepareBreadcrumbItems(
          activatedRouteSnapshot,
          routeLinkPrefix,
        );
      } else if (activatedRouteSnapshot?.routeConfig?.data?.['breadcrumb']) {
  ...
  private setQueryParamsForActiveBreadcrumb(
    ...
    if (lastItem) {
      ...
      if (fragment) lastItem.fragment = fragment;
    }
  }
  ...
  private updateStore(key: string, breadcrumb: BreadcrumbDefinition) {
    const storeItemIndex = this.dynamicBreadcrumbStore.findIndex((item) => {
      return (
        breadcrumb[key as keyof BreadcrumbDefinition] ===
        item[key as keyof BreadcrumbDefinition]
      );
    });
    if (storeItemIndex > -1) {
