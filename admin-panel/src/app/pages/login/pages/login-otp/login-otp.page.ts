...
export class LoginOtpPage implements OnInit {
      ...
      .pipe(
        switchMap((res) => {
          ...
          if (res.success) {
            ...
            return this.handlePlatformOwnerCheck();
          }
          return of(false);
        }),
        catchError((err) => {
  ...
  handleMultipleProfilesDetection(res: ILoginVerifyResponse) {
    ...
    const personnels = users.filter((user) => user.type === UserType.PERSONNEL);
    this.modalCtrl.dismiss();
    openProfilesDetedtedModal({
      modalCtrl: this.modalCtrl,
      phoneNumber: this.phoneNumberControl.value!,
      profilesList: personnels,
      code: this.otpCode(),
  ...
  private handlePlatformOwnerCheck(): Observable<boolean> {
    return this.authService.platformOwnerCheck().pipe(
      map(({ isPlatformOwner }) => {
        if (isPlatformOwner) {
        ...
        this.toaster.error(
          this.translocoService.translate('admin.access_denied.title'),
          this.translocoService.translate('admin.access_denied.description'),
        );
