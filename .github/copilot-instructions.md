...
- When creating Pull Requests, target the `sprint/stable` branch instead of `main`.
...
- **DTO transformations**: transformation helpers sit in `src/app/shared/dto-transformation` and specific feature `data-access` folders. Update both model and transform functions together when the payload shape changes. Each feature’s `data-access` folder must keep the full contract:
  - `*.dto.ts` files declare the backend response shape.
  - `*.interface.ts` files declare the app-facing model that components consume.
  - `*-dto-transform.ts` files convert between the DTO and interface.
  - Services fetch DTOs with `HttpClient`, run them through the transform, and expose only the interface to the rest of the app.
...
- **Tailwind utilities**:
  ...
  - **Modal utilities**: Use standardized modal classes for consistency:
    - `.ds-modal-title` — Single line/lg/high-emphasis with text-emphasis-high (for modal titles)
    - `.ds-modal-subtitle` — content/sm/mid-emphasis with text-emphasis-mid (for modal subtitles/descriptions)
    - `.ds-modal-close-btn` — Standard close button styling (24×24px, gray background, white icon, hidden on mobile, visible on desktop)
...
Adhering to these guidelines keeps the nx front end aligned with product expectations, reduces regressions, and ensures AI-generated contributions integrate smoothly.
